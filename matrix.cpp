...
/*  subroutine to initialize a two dimensional diagonal array to array values
inputs are:  2D array, number of rows, number of columns, and array of initial values.
*/

void mat_diag( double *in, int rows, int cols, double *init_val ) {

    int i, j;
    for( i=0; i<rows; i++ )
        for(j=0;j<cols;j++)
            if(i==j)
                in[i*rows+j] = init_val[i];
            else
                in[i*rows+j] = 0;
...
/* subroutine to multiply matrix by a constant value
 * B = c*A = A*c
 * inputs are: A, na (row in A), ma (cols in A),
 * c (constant to multiply by), and B, the scaled
 * output matrix
 */

void mat_mult_const( double *A, int na, int ma,
               double c, double *B ) {

    int i, j;

    mat_init( B, na, ma, 0.0 );
